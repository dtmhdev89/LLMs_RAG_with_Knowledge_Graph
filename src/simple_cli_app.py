from src.services.knowledge_graph_builder import KnowledgeGraphBuilder
...
if __name__ == "__main__":
    ...
    graph = knowledge_graph_builder.neo4j_graph

    chain = GraphCypherQAChain.from_llm(
        knowledge_graph_builder.llm,
        graph=graph,
        verbose=True,
    ...
    # interact with neo4j
    print("Start to interact with data by llm")

    while True:
        mode = input(
            """
            Select mode to run:
            1: for generate graph from input text and query
            2: query on current graph on the graph database
            0: exit
            """
        )

        if mode == '1':
            text = input("Input text to generate graph documents:")
            graph_documents, _net = knowledge_graph_builder.generate_knowledge_graph(
                text
            )
            knowledge_graph_builder.store_graph_to_db(graph_documents=graph_documents)
        elif mode == '0':
            break

        while True:
            query = input("Input to query with Neo4j: ")
            if query.strip() == "":
                continue
